...
class UpstoxBEPScanner:
    ...
    def detect_multi_touch_breakout(self, min_touches: int = 2) -> List[Dict]:
        """
        BEP MULTI-TOUCH BREAKOUT ALERT — v5.1 (with support candle confirmation)

        Detects when a strike has been repeatedly tested at BEP (2+ touches)
        and then breaks through, with a 3rd confirmation candle that retests
        the broken level as support/resistance.

        FLOW:
        Candle 1: Close crosses BEP (breakout start)
        Candle 2: Close stays on breakout side (confirmation)
        Candle 3: RETEST candle — wick touches back to BEP, but close holds
                  the breakout side. This proves the flipped level is strong.

        RESISTANCE → SUPPORT (BUY CE):
          - 2+ candles had high >= BEP (touched resistance zone)
          - Candle 1: close > BEP
          - Candle 2: close > BEP (consecutive)
          - Candle 3: low <= BEP (retest) AND close > BEP (held) AND open < close (green)
          → BUY CE

        SUPPORT → RESISTANCE (BUY PE):
          - 2+ candles had low <= BEP (touched support zone)
          - Candle 1: close < BEP
          - Candle 2: close < BEP (consecutive)
          - Candle 3: high >= BEP (retest) AND close < BEP (held) AND open > close (red)
          → BUY PE
        ...
        for key, df in self.candle_history.items():
            ...
            if len(df_mkt) < 4:  # Need 2 touches + 2 breakout candles + 1 retest
                continue

            # --- RESISTANCE → SUPPORT (BUY CE) ---
            # Need: 2+ resistance touches AND 2 consecutive closes > BEP AND retest candle
            resistance_touches = df_mkt[df_mkt['high'] >= bep]
            if len(resistance_touches) >= min_touches:
                for i in range(len(df_mkt) - 2):  # -2 because we need i, i+1, i+2
                    c1 = df_mkt.iloc[i]     # Breakout candle 1
                    c2 = df_mkt.iloc[i + 1] # Breakout candle 2 (consecutive)
                    c3 = df_mkt.iloc[i + 2] # RETEST candle (support confirmation)

                    # Candles 1 & 2: both close above BEP (breakout confirmed)
                    if not (c1['close'] > bep and c2['close'] > bep):
                        continue

                    # Candle 3: SUPPORT RETEST
                    # - Green candle (open < close) — bullish
                    # - Low touches or goes below BEP (retest)
                    # - Close stays above BEP (held support)
                    is_green = c3['open'] < c3['close']
                    retest_low = c3['low'] <= bep
                    held_support = c3['close'] > bep

                    if is_green and retest_low and held_support:
                        # Verify touches happened BEFORE the breakout candles
                        touch_before = df_mkt[
                        ...
                        if len(touch_before) >= min_touches:
                            results.append({
                                ...
                                'breakout_candle_1': c1['timestamp'].strftime('%H:%M'),
                                'breakout_candle_2': c2['timestamp'].strftime('%H:%M'),
                                'retest_candle': c3['timestamp'].strftime('%H:%M'),
                                'c1_close': c1['close'],
                                'c2_close': c2['close'],
                                'c3_open': c3['open'],
                                'c3_close': c3['close'],
            ...
            # --- SUPPORT → RESISTANCE (BUY PE) ---
            support_touches = df_mkt[df_mkt['low'] <= bep]
            if len(support_touches) >= min_touches:
                for i in range(len(df_mkt) - 2):
                    c1 = df_mkt.iloc[i]
                    c2 = df_mkt.iloc[i + 1]
                    c3 = df_mkt.iloc[i + 2]

                    # Candles 1 & 2: both close below BEP
                    if not (c1['close'] < bep and c2['close'] < bep):
                        continue

                    # Candle 3: RESISTANCE RETEST
                    # - Red candle (open > close) — bearish
                    # - High touches or goes above BEP (retest)
                    # - Close stays below BEP (held resistance)
                    is_red = c3['open'] > c3['close']
                    retest_high = c3['high'] >= bep
                    held_resistance = c3['close'] < bep

                    if is_red and retest_high and held_resistance:
                        touch_before = df_mkt[
                            (df_mkt['timestamp'] < c1['timestamp']) &
                            (df_mkt['low'] <= bep)
                        ]
                        if len(touch_before) >= min_touches:
                            results.append({
                                ...
                                'opt': opt,
                                'bep': bep,
                                'breakout_type': 'SUPPORT → RESISTANCE',
                                'touch_count': len(touch_before),
                                'breakout_candle_1': c1['timestamp'].strftime('%H:%M'),
                                'breakout_candle_2': c2['timestamp'].strftime('%H:%M'),
                                'retest_candle': c3['timestamp'].strftime('%H:%M'),
...
def _build_multi_touch_telegram_message(mt: Dict) -> str:
    ...
    bk1_time     = mt['breakout_candle_1']
    bk2_time     = mt['breakout_candle_2']
    retest_time  = mt['retest_candle']
    c3_open      = mt['c3_open']
    c3_close     = mt['c3_close']
    ...
    return (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "🕐 Breakout: " + bk1_time + " → " + bk2_time + "\n"
        "✅ " + retest_type + " Retest (" + retest_time + "): "
        "O:" + str(round(c3_open, 1)) + " C:" + str(round(c3_close, 1)) + "\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "📌 *Trade Hint:*\n"
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(
            ...
            "SUPPORT -> RESISTANCE: 2+ lows <= BEP, 2 closes < BEP, "
            "then red candle with high >= BEP and close < BEP -> BUY PE"
        )
                ...
                for mt in mt_alerts:
                    ...
                    if mt_id not in st.session_state.notified_mt_ids:
                        ...
                        notify(
                            ...
                            f"Retest at {mt['retest_candle']}: O:{mt['c3_open']:.1f} C:{mt['c3_close']:.1f}",
    ...
    if st.session_state.get('mt_enabled', True) and 'scanner' in st.session_state:
        ...
        if mt_alerts2:
            ...
            for mt in mt_alerts2:
                ...
                bk1 = mt['breakout_candle_1']
                bk2 = mt['breakout_candle_2']
                retest = mt['retest_candle']
                c3o = mt['c3_open']
                c3c = mt['c3_close']
                c3l = mt['c3_low']
                c3h = mt['c3_high']
                candle_emoji = "🟢" if action == 'BUY CE' else "🔴"
                with st.expander(
                    f"🔥 MULTI-TOUCH BREAKOUT — {candle_emoji} {action} "
                    ...
                    <div class="mt-box">
                        <b style="color:#ff5722;font-size:1.2rem;">
                            🔥 BEP MULTI-TOUCH BREAKOUT &nbsp;|&nbsp; {candle_emoji} {action}
                        </b><br>
                        <small>
                        ...
                        <span style="color:#ffd700;">
                            ✅ Retest ({retest}): O:{c3o:.1f} H:{c3h:.1f} L:{c3l:.1f} C:{c3c:.1f}
                        </span><br><br>
                        <b>📌 Trade Hint:</b><br>
                        <span style="color:#00e676;">
                            • {candle_emoji} {action} {strike}{opt}: Entry &gt; {bep:.2f}
                        </span>
                    </div>
                    """, unsafe_allow_html=True)
